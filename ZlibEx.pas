...
// **: Add overload to take old enum type
function ZCompressStr(const s: AnsiString; level: TCompressionLevel): AnsiString; overload;
...
const
  ...
  _z_errmsg: array[0..9] of AnsiString = (
...
begin
  ZCompressString(RawByteString(result), s, level);
end;

function ZCompressStr(const s: AnsiString; level: TCompressionLevel): AnsiString;
begin
...
var
  ...
  size: Integer;
begin
  ZCompress(Pointer(s), Length(s) * SizeOf(WideChar), buffer, size, level);
...
function ZCompressStrEx(const s: AnsiString;
  level: TZCompressionLevel): AnsiString;
begin
  ZCompressStringEx(RawByteString(result), s, level);
end;
...
function ZCompressStr2(const s: AnsiString; level: TZCompressionLevel;
  windowBits, memLevel: Integer; strategy: TZStrategy): AnsiString;
begin
  ZCompressString2(RawByteString(result), s, level, windowBits, memLevel, strategy);
end;
...
var
  ...
  size: Integer;
begin
  ZCompress2(Pointer(s), Length(s) * SizeOf(WideChar), buffer, size,
...
function ZCompressStrWeb(const s: AnsiString): AnsiString;
begin
  ZCompressStringWeb(RawByteString(result), s);
end;
...
begin
  inherited Create(string(_z_errmsg[2 - code]));
  ...
  try
    ...
    try
      ...
      begin
        ...
        strm.next_out := PAnsiChar(NativeInt(OutBuf) + (NativeInt(strm.next_out) - Integer(P)));
        strm.avail_out := 256;
      end;
    finally
